...
for i,col in enumerate(filter_columns):
    ...
    aux_combo = ttk.Combobox(root,values=['']+anisotropy_results[col].unique().tolist())
    filter_fields[col]=aux_combo
    aux_label.grid(row=i+1, column=0)
    aux_combo.grid(row=i+1, column=1, ipadx="100")

for col in ["N","max_length","cased","no_stop"]:
...
all_cols=['']+list(anisotropy_results.columns)
defaults={
    'x':all_cols.index("layer"),
    'y':all_cols.index("anisotropy"),
